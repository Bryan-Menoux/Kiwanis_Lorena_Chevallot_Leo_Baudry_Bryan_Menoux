...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    // Si tu as un champ rejected, on exclut les rejetés de la liste d’attente
    // Si ton champ a un autre nom, adapte juste le filter
    const users = await pbServer.collection("users").getFullList({
      sort: "-created",
      filter: "verified=false && (rejected=false || rejected=null)",
